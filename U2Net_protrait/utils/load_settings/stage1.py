...
# load setting.json
with open("utils/load_settings/train_stage1_settings.json", "r") as file:
	settings = json.load(file)

# load items in setting file
project_name, model_name = settings["project_name"], settings["model_name"]
dataset_path = settings["dataset_path"]
batch_size, epoch_num, image_num = settings["batch_size"], settings["epoch_num"], settings["image_num"]
...
full_project_name = time.strftime(project_name + "__%Y_%m_%d__%H_%M_%S", time.localtime()) + "/"
result_dir = os.path.join("results_stage1", full_project_name)
