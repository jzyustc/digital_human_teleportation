...
class Config(object):
    ...
    # Path to models imagenet model
    IMAGENET_MODEL_PATH = os.path.join(os.getcwd(), "resnet50_imagenet.pth")

    # NUMBER OF GPUs to use. For CPU use 0
    GPU_COUNT = 1

    # Number of results to train with on each GPU. A 12GB GPU can typically
    # handle 2 results of 1024x1024px.
    ...
    # memory load. Recommended when using high-resolution results.
    USE_MINI_MASK = True
    ...
    # If True, pad results with zeros such that they're (max_dim by max_dim)
