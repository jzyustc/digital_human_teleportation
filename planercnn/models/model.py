...
class MaskRCNN(nn.Module):
    ...
    def detect(self, images, camera, mold_image=True, image_metas=None):
        ...
        results: List of results, potentially of different sizes.
...
def mold_inputs(config, images):
    """Takes a list of results and modifies them to the format expected
    as an input to the neural network.
    results: List of image matricies [height,width,depth]. Images can have
        different sizes.
...
def compose_image_meta(image_id, image_shape, window, active_class_ids):
    ...
    active_class_ids: List of class_ids available in the dataset from which
        the image came. Useful if training on results from multiple datasets
...
def mold_image(images, config):
    """Takes RGB results with 0-255 values and subtraces
