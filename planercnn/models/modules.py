...
def unmoldDetections(config, camera, detections, detection_masks, depth_np, unmold_masks=True, debug=False):
	...
	if config.NUM_PARAMETER_CHANNELS > 0:
		...
		parameters_array = detection_masks[torch.arange(len(detection_masks)).cuda().long(),
		...
		for detectionIndex in range(len(detections)):
			...
			if (box[2] - box[0]) * (box[3] - box[1]) <= 0:
				continue
			parameters = F.upsample(parameters_array[detectionIndex].unsqueeze(0),
									size=(box[2] - box[0], box[3] - box[1]), mode='bilinear').squeeze(0)
			final_parameters = torch.zeros(config.NUM_PARAMETER_CHANNELS, config.IMAGE_MAX_DIM,
	...
	if 'normal' in config.ANCHOR_TYPE:
		...
		if config.NUM_PARAMETER_CHANNELS == 4:
			...
			offsets = ((plane_normals.view(-1, 3, 1, 1) * XYZ_np_cropped).sum(1, keepdim=True) * masks_cropped).sum(
				-1).sum(-1) / mask_sum
			...
			if config.FITTING_TYPE % 2 == 1:
				...
				plane_parameters = torch.stack(
					[torch.matmul(torch.inverse(AA[planeIndex]), Ab[planeIndex]) for planeIndex in range(len(AA))],
					dim=0)
				...
				offsets = ((plane_normals.view(-1, 3, 1, 1) * XYZ_np_cropped).sum(1) * masks_cropped).sum(-1).sum(
					-1) / torch.clamp(masks_cropped.sum(-1).sum(-1), min=1e-4)
				plane_parameters = plane_normals * offsets.view((-1, 1))
				pass
			pass
...
def warpModuleDepth(config, camera, depth_1, features_2, extrinsics_1, extrinsics_2, width, height):
	...
	warped_features, valid_mask = warpModuleXYZ(config, camera, XYZ_1.unsqueeze(2), features_2, extrinsics_1,
												extrinsics_2, width, height)
...
def warpModuleXYZ(config, camera, XYZ_1, features_2, extrinsics_1, extrinsics_2, width, height):
	...
	XYZ_2 = torch.matmul(torch.matmul(torch.cat([XYZ_1, torch.ones((len(XYZ_1), 1)).cuda()], dim=-1),
									  extrinsics_1.inverse().transpose(0, 1)), extrinsics_2.transpose(0, 1))
	...
	warped_features = warped_features.view((numFeatureChannels, height, width, numPlanes)).transpose(2, 3).transpose(1,
																													 2).transpose(
		0, 1).contiguous().view((-1, int(features_2.shape[1]), height, width))
	...
	if one_hot:
		for detectionIndex in range(len(detections)):
			...
			if config.FITTING_TYPE >= 2:
				if (torch.norm(planeXYZ[:, :, :, detectionIndex] - XYZ_np, dim=0) * mask_binary).sum() / torch.clamp(
						mask_binary.sum(), min=1e-4) > 0.5:
...
class ConvBlock(torch.nn.Module):
	...
	def __init__(self, in_planes, out_planes, kernel_size=1, stride=1, padding=0, output_padding=0, mode='conv',
				 use_bn=True):
		super(ConvBlock, self).__init__()
		self.use_bn = use_bn
		if mode == 'conv':
			self.conv = torch.nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding,
										bias=not self.use_bn)
		elif mode == 'deconv':
			self.conv = torch.nn.ConvTranspose2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
												 padding=padding, output_padding=output_padding, bias=not self.use_bn)
		elif mode == 'upsample':
			self.conv = torch.nn.Sequential(torch.nn.Upsample(scale_factor=stride, mode='nearest'),
											torch.nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=1,
															padding=padding, bias=not self.use_bn))
		elif mode == 'conv_3d':
			self.conv = torch.nn.Conv3d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding,
										bias=not self.use_bn)
		elif mode == 'deconv_3d':
			self.conv = torch.nn.ConvTranspose3d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
												 padding=padding, output_padding=output_padding, bias=not self.use_bn)
		else:
...
class PlaneToDepthLayer(torch.nn.Module):
	...
	def forward(self, intrinsics, plane, mask):
		...
		:param rot:   n x 3
		:param trans: n x3
		:param shared_K: if True, we share intrinsics for the depth results of the whole batch
